...
// Render Win/Lose/Draw image
function renderResultImage() {
    const imgWidth = 300;
    const imgHeight = 400;
    const imgX = (cvs.width - 50) - imgWidth;
    const imgY = cvs.height / 2 - imgHeight / 2; // Adjust position below the result text
    // Đảm bảo ảnh đã tải xong trước khi vẽ
    resultImage.onload = () => {
        ctx.drawImage(resultImage, imgX, imgY, imgWidth, imgHeight);
    };
}
...
const drawSound = new Audio("Player_draw.mp3");
...
function compare(computer_value, player_value, goal_number) {
    ...
    if (computerDiff < playerDiff) {
        resultImage.src = "Player_lose.jpg";
        loseSound.play(); // Phát âm thanh thua
        return "Computer Won!";
    }
    
    if (computerDiff === playerDiff) {
        resultImage.src = "Player_draw.jpg";
        drawSound.play(); // Phát âm thanh hòa
        return "Draw!";
    }
...
<<<<<<< HEAD
    ...
    playerDiv.style.top = `${cvs.height / 4 - 50}px`;
    document.body.appendChild(playerDiv);

    let computerDiv = document.createElement('div');
    computerDiv.classList.add('dice-result');
